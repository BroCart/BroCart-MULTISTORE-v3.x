...
class ModelExtensionShippingAutolux extends Model {
    ...
    function getQuote($address) {
        ...
        $image = HTTP_SERVER.'image/catalog/shipping/autolux.jpg';
        ...
        if ($status) {
            ...
            $quote_data['autolux'] = array(
                ...
                'text' => $this->currency->format($this->tax->calculate($this->config->get('shipping_autolux_cost'), $this->config->get('shipping_autolux_tax_class_id'), $this->config->get('shipping_config_tax')), $this->session->data['currency'])
            );
