...
class ModelExtensionShippingDeliveri extends Model {
    ...
    function getQuote($address) {
        ...
        $image = HTTP_SERVER.'image/catalog/shipping/delivery.jpg';
        ...
        if ($status) {
            ...
            $quote_data['deliveri'] = array(
                ...
                'cost' => $this->config->get('shipping_deliveri_cost'),
                'tax_class_id' => $this->config->get('shipping_deliveri_tax_class_id'),
                'text' => $this->currency->format($this->tax->calculate($this->config->get('shipping_deliveri_cost'), $this->config->get('shipping_deliveri_tax_class_id'), $this->config->get('shipping_config_tax')), $this->session->data['currency'])
            );
