...
class ModelExtensionShippingMeest extends Model {
    ...
    function getQuote($address) {
        ...
        $image = HTTP_SERVER.'image/catalog/shipping/meest.jpg';
        ...
        if ($status) {
            ...
            $quote_data['meest'] = array(
                ...
                'cost' => $this->config->get('shipping_meest_cost'),
                'tax_class_id' => $this->config->get('shipping_meest_tax_class_id'),
                'text' => $this->currency->format($this->tax->calculate($this->config->get('shipping_meest_cost'), $this->config->get('shipping_meest_tax_class_id'), $this->config->get('shipping_config_tax')), $this->session->data['currency'])
            );
