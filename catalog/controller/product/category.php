		...
		if ($category_info) {			
			if ($page > 1) {				
				if ($category_info['meta_title']) {
					$this->document->setTitle($category_info['meta_title'] . $this->language->get('text_page') . $page);
				} else {
					$this->document->setTitle($category_info['name'] . $this->language->get('text_page') . $page);
				}
				if ($category_info['meta_description']) {
					$this->document->setDescription($category_info['meta_description'] . $this->language->get('text_page') . $page);
				}
				$data['description'] = '';
			} else {
				if ($category_info['meta_title']) {
				...
				if ($category_info['image'] && is_file(DIR_IMAGE . $category_info['image'])) {
					$this->document->setOgImage(HTTP_SERVER . 'image/' . $category_info['image']);
				}				
