	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['meta_title']) {
				$this->document->setTitle(trim($product_info['meta_title']));
			} else {
				$this->document->setTitle(trim($product_info['name']));
			}
			if ($product_info['image'] && is_file(DIR_IMAGE . $product_info['image'])) {
				$this->document->setOgImage(HTTP_SERVER . 'image/' . $product_info['image']);
			}
			...
			if ($product_info['meta_h1']) {
				$data['heading_title'] = trim($product_info['meta_h1']);
			} else {
