...
class ControllerProductManufacturer extends Controller {
	...
	public function info() {
		...
		if (isset($this->request->get['sort'])) {
			...
			$this->document->addRobots('noindex, follow');
		} else {
			$sort = 'p.sort_order';
		}
		...
		if ($manufacturer_info) {
			if ($page > 1) {
				if ($manufacturer_info['meta_title']) {
					$this->document->setTitle($manufacturer_info['meta_title'].$this->language->get('text_page').$page);
				} else {
					$this->document->setTitle($manufacturer_info['name'].$this->language->get('text_page').$page);
				}
				if ($manufacturer_info['meta_description']) {
					$this->document->setDescription($manufacturer_info['meta_description'].$this->language->get('text_page').$page);
				}
				$data['description'] = '';
			} else {
				if ($manufacturer_info['meta_title']) {
				...
				if ($manufacturer_info['meta_description']) {
					...
					$this->document->setOpengraph('og:description', $manufacturer_info['meta_description']);
				}
				if ($manufacturer_info['image'] && is_file(DIR_IMAGE . $manufacturer_info['image'])) {
					$this->document->setOgImage(HTTP_SERVER . 'image/' . $manufacturer_info['image']);
				}
			...
			if ($product_total && $limit && $page > 1) {
				if (!isset($this->session->data['maxpage'])) {	
					$this->session->data['maxpage'] = ceil($product_total / $limit);
				}		
			}
