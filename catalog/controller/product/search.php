	...
	public function ajax()
        {
                    ...
                    if( isset($this->request->get['keyword']) ) {
						...
						$keywords = mb_convert_case($this->request->get['keyword'], MB_CASE_LOWER, "UTF-8");
                        ...
                        if( strlen($keywords) >= 3 ) {
                            ...
                            $add = substr( $add, 4 );
							$sql  = 'SELECT pd.product_id, pd.name, p.model,p.price,p.image, p.tax_class_id FROM ' . DB_PREFIX . 'product_description AS pd ';
							...
							if ($values['image']) {
							$resim = $this->model_tool_image->resize($values['image'], 50,50);
							} else {
								$resim= '';
							}        
								...
								$fiyat = $this->currency->format($this->tax->calculate($values['price'], $values['tax_class_id'], $this->config->get('config_tax')));
									$data[$key] = array(
										...
										'resim' => $resim,
										'name' => htmlspecialchars_decode("   ".$values['name'] ." : ". $fiyat, ENT_QUOTES),
