...
class ControllerMailGdpr extends Controller {
	...
	public function index(&$route, &$args, &$output) {
		...
		$data['logo'] = '';
		if (is_array($logos)) {
			if (is_file(DIR_IMAGE . html_entity_decode($logos[$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8'))) {
				$data['logo'] = $this->model_tool_image->resize(html_entity_decode($logos[$this->config->get('config_language_id')], ENT_QUOTES, 'UTF-8'), $this->config->get('theme_default_image_location_width'), $this->config->get('theme_default_image_cart_height'));
			}
		} else {
