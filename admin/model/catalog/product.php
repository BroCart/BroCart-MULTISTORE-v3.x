...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', u_title = '" . $this->db->escape($value['u_title']) . "', u_h1 = '". $this->db->escape($value['u_h1']) ."',u_h2 = '". $this->db->escape($value['u_h2']) ."', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "'");
		}
		...
		if (isset($data['label'])) {
			foreach ($data['label'] as $label) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "label_to_product SET product_id = '" . (int)$product_id . "', label_id = '" . (int)$label['lable_id'] . "', date_begin = '" . $this->db->escape($label['date_begin']) . "', date_end = '" . $this->db->escape($label['date_end']) . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "label_to_product WHERE product_id = '" . (int)$product_id . "'");
		...
		//$buf = print_r($data['product_category'] ,true);$this->log->write($buf);
