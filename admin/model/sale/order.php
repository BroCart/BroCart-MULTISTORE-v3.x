...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.shipping_method, (SELECT text FROM " . DB_PREFIX . "order_total WHERE order_id = o.order_id AND code='shipping') as shipping_costs, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
	...
	public function addOrderHistory($order_id, $data) {
		...
		if ($this->config->get('config_cancel_status_id') == $data['order_status_id']) {	
			$product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");		
			foreach($product_query->rows as $product) {
				$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
