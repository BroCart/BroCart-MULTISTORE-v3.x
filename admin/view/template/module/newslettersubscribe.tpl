...
<tr>
        ...
        <tr>
          ...
          <span class="help"> Опция для зарегистрированных пользователей, которые так же могут подписаться или отписаться.
        ...
        <tr>
          <td><?php echo $text_kypon; ?> </td>
          <td> 
          <?php $tmp_option_list = $coupons; ?>
          <select name="newslettersubscribe_option_field"> 
			<option value="0" selected="selected"><?php echo $text_none; ?></option>
              <?php foreach($tmp_option_list as $key=>$opt) {				
                  if($opt['coupon_id'] == $newslettersubscribe_option_field){
                    echo("<option value='".$opt['coupon_id']."' selected='selected'>".$opt['name']."</option>");
                  }else{
                    echo("<option value='".$opt['coupon_id']."'>".$opt['name']."</option>");
                  }} ?>                 
                </select> 
          </td>
        </tr>		
        <tfoot>
         <?php $field_var = "newslettersubscribe_option_field1"; ?>
            <tr>
              <td> <?php echo $text_titlemod; ?> </td>
              <td> 
              <input type='text' name='newslettersubscribe_option_field1' value='<?php echo($$field_var); ?>'>
               </td>
            </tr>
        </tfoot>
      </table>
