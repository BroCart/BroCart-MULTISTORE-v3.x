...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      <h1><img src="view/image/product.png" alt="" /><?php echo $heading_title; ?><span class="infoname"></span></h1>
      <div class="buttons"><a onclick="$('#form').submit();" class="button"><?php echo $button_save; ?></a><a href="<?php echo $cancel; ?>" class="button"><?php echo $button_cancel; ?></a><a id="save_edit" class="button"><?php echo $text_apply; ?></a>
	  </div>
    </div>
    <div class="content">
      <div id="tabs" class="htabs"><a href="#tab-general"><?php echo $tab_general; ?></a><a href="#tab-data"><?php echo $tab_data; ?></a><a href="#tab-links"><?php echo $tab_links; ?></a><a href="#tab-attribute"><?php echo $tab_attribute; ?></a><a href="#tab-option"><?php echo $tab_option; ?></a><a href="#tab-discount"><?php echo $tab_discount; ?></a><a href="#tab-special"><?php echo $tab_special; ?></a><a href="#tab-image"><?php echo $tab_image; ?></a><a href="#tab-label"><?php echo $tab_label; ?></a><a href="#tab-product_tab"><?php echo $tab_product_tab; ?></a><a href="#tab-reward"><?php echo $tab_reward; ?></a><a href="#tab-design"><?php echo $tab_design; ?></a></div>
	   ...
	   <input type="hidden" name="save_continue" value="0" id="save_continue">
          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
              <tr>
                ...
                <td><input type="text" name="product_description[<?php echo $language['language_id']; ?>][name]" size="100" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?>" id="prod"/>
                  <?php if (isset($error_name[$language['language_id']])) { ?>
                  <span class="error"><?php echo $error_name[$language['language_id']]; ?></span>
                  <?php } ?></td>
              </tr>
			  <tr>
                <td><?php echo $text_titles; ?></td>
			...
			<tr>
			 ...
			 <tr>
				<td><?php echo $text_autometa; ?></td>
				<td><input type="checkbox" name="used_auto_meta[<?php echo $language['language_id']; ?>]" value = "checked" <?php echo isset($useo_auto_meta[$language['language_id']]) ? $useo_auto_meta[$language['language_id']] : ''; ?> />
				</td>
			</tr>
              <tr>
          ...
          <table class="form">
            ...
            <tr>
			  ...
			  <select name="location">
                  ...
                  <?php foreach ($info as $info_class) { ?>
                  <?php if ($info_class['info_id'] == $location) { ?>
                  <option value="<?php echo $info_class['info_id']; ?>" selected="selected"><?php echo $info_class['title']; ?></option>
                  <?php } else { ?>
                  <option value="<?php echo $info_class['info_id']; ?>"><?php echo $info_class['title']; ?></option>
                  <?php } ?>
                  <?php } ?>
                </select>
			  </td>
            ...
            <tr>
              ...
              <td><input type="text" name="keyword" value="<?php echo $keyword; ?>" id="keyw"/></td>
            </tr>
            <tr>
        ...
        <div id="tab-option">
          ...
          <div id="tab-option-<?php echo $option_row; ?>" class="vtabs-content">
            ...
            <?php if ($product_option['type'] == 'select' || $product_option['type'] == 'mixed' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') { ?>
		...
		<div id="tab-label"> 
			<table class="form">
            <tr>
              <td><?php echo $entry_labels; ?></td>
              <td>
			  <select id="komp" name="accessory">
                  <option value="0" selected="selected"><?php echo $text_none; ?></option>
				<?php foreach ($labels as $label) { ?>                 
                  <option value="<?php echo $label['lable_id']; ?>"><?php echo $label['stick_text']; ?></option>                  
                  <?php } ?>
                </select>
				</td>
            </tr>
          </table>
          <table id="label" class="list">
            <thead>
              <tr>
                <td class="left"><?php echo $entry_labels_name; ?></td>
                <td class="right"><?php echo $entry_date_start; ?></td>
				<td class="right"><?php echo $entry_date_end; ?></td>
				<td class="right"><?php echo $entry_del; ?></td>
              </tr>
            </thead>
			<?php $label_row = 0; ?>
            <?php foreach ($labelpro as $labelp) { ?>
            <tbody id="label-row<?php echo $label_row; ?>">
              <tr id="product-accessory" >
                <td class="left"><?php echo $labelp['stick_text']; ?></td>
                <td class="right"><input type="text" name="label[<?php echo $label_row; ?>][date_begin]" value="<?php echo $labelp['date_begin']; ?>" class="date" /></td>
				<td class="right"><input type="text" name="label[<?php echo $label_row; ?>][date_end]" value="<?php echo $labelp['date_end']; ?>" class="date" /></td>
				<td class="right"><a onclick="$('#label-row<?php echo $label_row; ?>').remove();" class="button"><?php echo $button_remove; ?></a></td>
				<input type="hidden" name="label[<?php echo $label_row; ?>][lable_id]" value="<?php echo $labelp['lable_id']; ?>" />
              </tr>
            </tbody>
			<?php $label_row ++; ?>
            <?php } ?>	
...
$('#'+id).html(val);
	...
	$infoname = $('input[name="product_description[<?php echo $this->config->get('config_language_id'); ?>][name]"]'); 
	$title = $('.infoname');
		if ($infoname.val() != '') {
...
$('input[name=\'option\']').catcomplete({
	...
	select: function(event, ui) {
		...
		if (ui.item.type == 'select' || ui.item.type == 'mixed' || ui.item.type == 'radio' || ui.item.type == 'checkbox' || ui.item.type == 'image') {
...
//--></script>
<script type="text/javascript"><!--
var label_row = <?php echo $label_row; ?>;

$("select[name=\'accessory\']").change(function () {
	var currentText = $(this).find(":selected").text();
    var value = $('#komp').val();

	html  = '<tbody id="label-row' + label_row + '">';
	html += '  <tr>'; 
	html += '    <td class="left">' + currentText + '</td>';   
    html += '    <td class="right"><input type="text" name="label[' + label_row + '][date_begin]" value="" class="date" /></td>';
	html += '    <td class="right"><input type="text" name="label[' + label_row + '][date_end]" value="" class="date" /></td>';    
	html += '    <td class="right"><a onclick="$(\'#label-row' + label_row + '\').remove();" class="button"><?php echo $button_remove; ?></a></td>';
	html += '    <input type="hidden" name="label[' + label_row + '][lable_id]" value="'+ value +'" />';
	html += '  </tr>';
    html += '</tbody>';
	
	$('#label tfoot').before(html);
	
	$('#label-row' + label_row + ' .date').datepicker({dateFormat: 'yy-mm-dd'});
...
<script type="text/javascript"><!--
$('input#prod').translit('send', 'input#keyw');
//--></script> 
<?php } ?> 
<?php echo $footer; ?>