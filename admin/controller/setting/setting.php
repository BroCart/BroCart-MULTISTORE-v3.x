...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		foreach ($data['languages'] as $key => $result) {
			if (isset($this->request->post['config_meta_title_' . $result['language_id']])) {
				$data['config_meta_title'][$result['code']] = $this->request->post['config_meta_title_' . $result['language_id']];
			} else {
				$data['config_meta_title'][$result['code']] = $this->config->get('config_meta_title_' . $result['language_id']);
			}
			if (isset($this->request->post['config_meta_description_' . $result['language_id']])) {
				$data['config_meta_description'][$result['code']] = $this->request->post['config_meta_description_' . $result['language_id']];
			} else {
				$data['config_meta_description'][$result['code']] = $this->config->get('config_meta_description_' . $result['language_id']);
			}

			if (isset($this->request->post['config_meta_keyword_' . $result['language_id']])) {
				$data['config_meta_keyword'][$result['code']] = $this->request->post['config_meta_keyword_' . $result['language_id']];
			} else {
		...
		if (!is_array($data['config_logo'])) {
			$data['config_logo'] = array();			
			foreach ($data['languages'] as $language) {
				$data['config_logo'][$language['language_id']] = "";
			}
		}		
		$data['logo'] = array();
		foreach ($data['config_logo'] as $key => $logo) {
			if (is_file(DIR_IMAGE . $logo)) {
				$data['logo'][$key] = $this->model_tool_image->resize($logo, 100, 100);
			} else {
				$data['logo'][$key] = $this->model_tool_image->resize('no_image.png', 100, 100);
			}
		}
		...
		if (isset($this->request->post['config_watermark_global'])) {
			$data['config_watermark_global'] = $this->request->post['config_watermark_global'];
		} else {
			$data['config_watermark_global'] = $this->config->get('config_watermark_global');
		}
		
		if (isset($this->request->post['config_watermark'])) {
			$data['config_watermark'] = $this->request->post['config_watermark'];
		} else {
			$data['config_watermark'] = $this->config->get('config_watermark');
		}

		if (isset($this->request->post['config_watermark']) && is_file(DIR_IMAGE . $this->request->post['config_watermark'])) {
			$data['watermark'] = $this->model_tool_image->resize($this->request->post['config_watermark'], 100, 100);
		} elseif ($this->config->get('config_watermark') && is_file(DIR_IMAGE . $this->config->get('config_watermark'))) {
			$data['watermark'] = $this->model_tool_image->resize($this->config->get('config_watermark'), 100, 100);
		} else {
			$data['watermark'] = $this->model_tool_image->resize('catalog/logo_bro_adm.png', 100, 100);
		}
		...
		if (isset($this->request->post['config_simple_order'])) {
			$data['config_simple_order'] = $this->request->post['config_simple_order'];
		} else {
			$data['config_simple_order'] = $this->config->get('config_simple_order');
		}
		
		if (isset($this->request->post['config_marketplace'])) {
			$data['config_marketplace'] = $this->request->post['config_marketplace'];
		} else {
			$data['config_marketplace'] = $this->config->get('config_marketplace');
		}
	...
	protected function validate() {
		...
		foreach ($this->model_localisation_language->getLanguages() as $result) {
			if ((utf8_strlen($this->request->post['config_meta_title_' . $result['language_id']]) < 3) || (utf8_strlen($this->request->post['config_meta_title_' . $result['language_id']]) > 70)) {
