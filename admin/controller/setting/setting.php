...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['main_title']) || ($this->request->post['main_h1']) || ($this->request->post['main_meta_desc']) || ($this->request->post['main_meta_keyw']) || ($this->request->post['main_desc'])) {
				$this->model_setting_setting->editMain($this->request->post);				
			}
		...
		$this->data['entry_yandex_metrika'] = $this->language->get('entry_yandex_metrika');
		$this->data['button_save'] = $this->language->get('button_save');
		$this->data['button_cancel'] = $this->language->get('button_cancel');
		$this->data['entry_h1'] = $this->language->get('entry_h1');
		$this->data['entry_full_desc'] = $this->language->get('entry_full_desc');
		...
		$quer = $this->db->query("SELECT language_id FROM " . DB_PREFIX . "seo_main");
		
		$bp = $this->db->query("SELECT language_id FROM " . DB_PREFIX . "language WHERE status = '1'");
		...
		if (($t == 0) || ($t == array())){							
					foreach ($glob2 as $lang) {
						$query = $this->db->query("INSERT INTO `" . DB_PREFIX . "seo_main` SET language_id = '" . (int)$lang['language_id'] . "'");	
						}
		} elseif(($t != $s) && ($t != 0)){
					foreach ($glob1 as $lang) {
						$query = $this->db->query("DELETE FROM " . DB_PREFIX . "seo_main WHERE language_id = '" . (int)$lang['language_id'] . "'");	
					}
		foreach ($glob2 as $lang) {
						$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_main` SET language_id = '" . (int)$lang['language_id'] . "'");
					}
		}
					
				if (isset($this->request->post['main_title'])) {
					
					$this->data['main_title'] = $this->request->post['main_title'];			
				} else {
					$this->data['main_title'] = $this->model_setting_setting->getMainInfo($this->config->get('config_language'));			
				}
				
				if (isset($this->request->post['main_h1'])) {		
					$this->data['main_h1'] = $this->request->post['main_h1'];			
				} else {
					$this->data['main_h1'] = $this->model_setting_setting->getMainInfo($this->config->get('config_language'));			
				}
				
				if (isset($this->request->post['main_meta_desc'])) {		
					$this->data['main_meta_desc'] = $this->request->post['main_meta_desc'];			
				} else {
					$this->data['main_meta_desc'] = $this->model_setting_setting->getMainInfo($this->config->get('config_language'));			
				}
				
				if (isset($this->request->post['main_meta_keyw'])) {		
					$this->data['main_meta_keyw'] = $this->request->post['main_meta_keyw'];			
				} else {
					$this->data['main_meta_keyw'] = $this->model_setting_setting->getMainInfo($this->config->get('config_language'));			
				}
				
				if (isset($this->request->post['main_desc'])) {		
					$this->data['main_desc'] = $this->request->post['main_desc'];			
				} else {
					$this->data['main_desc'] = $this->model_setting_setting->getMainInfo($this->config->get('config_language'));			
				}
		...
		if (isset($this->request->post['config_yandex_metrika'])) {
			$this->data['config_yandex_metrika'] = $this->request->post['config_yandex_metrika']; 
		} else {
			$this->data['config_yandex_metrika'] = $this->config->get('config_yandex_metrika');
		}
	...
	protected function validate() {
		...
		if (!$this->request->post['main_title']) {
