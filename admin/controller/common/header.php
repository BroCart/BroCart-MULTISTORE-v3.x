...
class ControllerCommonHeader extends Controller {
	protected function index() {
		...
		$this->data['text_stick'] = $this->language->get('text_stick');
		...
		$this->data['text_cfg'] = $this->language->get('text_cfg');
		...
		if (!$this->user->isLogged() || !isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$this->data['seo'] = $this->url->link('common/seogen', 'token=' . $this->session->data['token'], 'SSL');
			...
			$this->data['lables'] = $this->url->link('catalog/lables', 'token=' . $this->session->data['token'], 'SSL');
			...
			$this->data['link_lanedit'] = $this->url->link('module/translate', 'token=' . $this->session->data['token']);
			...
			$this->data['bro_conf'] = $this->url->link('module/bro_conf', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->load->model( 'localisation/language' );
                ...
                $list       = '';
                $i          = 0;
                $current    = $this->config->get( 'config_admin_language' );

                $this->data['token'] = ( !empty( $this->session->data['token'] ) ? $this->session->data['token'] : '' );

                // get a possible existing cookie
                if( !empty( $_COOKIE['adminLanguage'] ) ) {
                    if( $_COOKIE['adminLanguage'] != $current ) {
                        $current = $_COOKIE['adminLanguage'];
                    }
                }
                ...
                if( $i != 0 ) {
                    $list = '<ul class="right">' . "\n"
                    . '<li id="langSelector">' . "\n"
                    . '<select name="language" onchange="setLanguage(this.value);" style="margin: 7px 0 0 10px;">' . "\n";

                    foreach( $languages as $language ) {
                        $list .= '<option value="' . $language['code'] . '"'
                            . ( $language['code'] == $current ? ' selected="selected"' : '' )
