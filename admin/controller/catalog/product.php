	...
	private $error = array(); 
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->load->model('catalog/seotranslit');
			$this->request->post = $this->model_catalog_seotranslit->translit($this->request->post);
			...
			if ($this->request->post["save_continue"]=="0"){$this->redirect($this->url->link('catalog/product', 'token=' .$this->session->data['token'] . $url, 'SSL'));
		}
    	}
	...
	//end_of_func_edit_list
  	protected function getList() {				
		...
		if (isset($this->request->get['sort'])) {
			...
			$sort = 'p.date_added';//$sort = 'pd.name';
		}
  	...
  	protected function getForm() {
		...
		$this->data['text_autometa'] = $this->language->get('text_autometa');
		$this->data['text_titles'] = $this->language->get('text_titles');
		$this->data['text_h1'] = $this->language->get('text_h1');
		$this->data['text_h2'] = $this->language->get('text_h2');
		...
		$this->data['entry_del'] = $this->language->get('entry_del');
		$this->data['entry_labels_name'] = $this->language->get('entry_labels_name');
		...
		if ($this->config->get('useo_meta_num') ) 
		...
		$this->data['hide_upc'] = $this->config->get('hide_upc');
    	$this->data['hide_ean'] = $this->config->get('hide_ean');
    	$this->data['hide_jan'] = $this->config->get('hide_jan');
    	$this->data['hide_isbn'] = $this->config->get('hide_isbn');
		$this->data['hide_mpn'] = $this->config->get('hide_mpn');
		$this->data['loc_change'] = $this->config->get('loc_change');
		...
		$this->data['labels'] = $this->model_catalog_lables->getLables();
		
		if (isset($this->request->post['labelpro'])) {
      		$this->data['labelpro'] = $this->request->post['labelpro'];
		} elseif (isset($product_info)) {
			$this->data['labelpro'] = $this->model_catalog_lables->getLablesPro($this->request->get['product_id']);
		} else {
      		$this->data['labelpro'] = array();
    	}
		...
		foreach ($product_options as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'mixed' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				foreach ($product_options as $product_option) {
					...
					if ($option_info) {				
						if ($option_info['type'] == 'select' || $option_info['type'] == 'mixed' || $option_info['type'] == 'radio' || $option_info['type'] == 'checkbox' || $option_info['type'] == 'image') {
