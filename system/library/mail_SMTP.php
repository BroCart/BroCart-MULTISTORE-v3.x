...
include_once(DIR_SYSTEM . 'config/phpmailer/class.phpmailer.php');
...
class Mail {
	...
	/* public function send() {
		if (!$this->to) {
			trigger_error('Ошибка: E-Mail обязательно для заполнения!');
			exit();			
		}

		if (!$this->from) {
			trigger_error('Ошибка: E-Mail отправитель обязательно для заполнения!');
			exit();					
		}

		if (!$this->sender) {
			trigger_error('Ошибка: E-Mail получатель обязательно для заполнения!');
			exit();					
		}

		if (!$this->subject) {
			trigger_error('Ошибка: E-Mail текст обязательно для заполнения!');
			exit();					
		}

		if ((!$this->text) && (!$this->html)) {
			trigger_error('Ошибка: E-Mail сообщение обязательно для заполнения!');
			exit();					
		}
			...
			if (!$handle) {
				...
				if (substr($this->hostname, 0, 3) == 'tls') {
					...
					if (substr($reply, 0, 3) != 220) {
						trigger_error('Ошибка: STARTTLS not accepted from server!');
						exit();								
					}
				}
